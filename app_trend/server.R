...
data <- readRDS("../data/preprocess_minute_data.RDS")
...
shinyServer(function(input, output, session) {
  ...
  #     filter(adherent_day) %>% 
  #     group_by(id) %>% 
  #     summarise(sedentary_prop_mu = mean(sedentary_prop), 
  #               lipa_prop_mu = mean(lipa_prop), 
  #               mvpa_prop_mu = mean(mvpa_prop),
  #               sedentary_bout_mu = mean(sedentary_bout_mean))
  # })
  # 
  output$trend_filter <- renderPlot({
    ggplot(reactive_data()$daily_data, 
           aes(x=date, y=trend, color=is.na(!!sym(input$trend_input)))) + 
      geom_line(aes(group=1), 
                size = 1.2, alpha = 0.6)+
      scale_color_manual(values=c("red", "grey"), labels=c("adherent", "non-adherent")) + 
      geom_point(data = reactive_data()$daily_data, 
                 aes(x=date, y=!!sym(input$trend_input)), 
