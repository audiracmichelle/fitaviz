...
daytime_wear_summary <- minute_data %>% 
  ...
  summarise(
    ...
    sedentary_hours__DWT = sum(intensity == 0) / 60, 
    light_hours__DWT = sum(intensity == 1) / 60, 
    moderate_hours__DWT = sum(intensity == 2) / 60, 
    active_hours__DWT = sum(intensity == 3) / 60
  )
...
adherent_wear_summary <- minute_data %>% 
  ...
  summarise(
    ...
    sedentary_hours__AWT = sum(intensity == 0) / 60, 
    light_hours__AWT = sum(intensity == 1) / 60, 
    moderate_hours__AWT = sum(intensity == 2) / 60, 
    active_hours__AWT = sum(intensity == 3) / 60
  )
...
nonadh_wear_summary <- minute_data %>% 
  ...
  summarise(
    ...
    sedentary_hours__NAW = sum(intensity == 0) / 60, 
    light_hours__NAW = sum(intensity == 1) / 60, 
    moderate_hours__NAW = sum(intensity == 2) / 60, 
    active_hours__NAW = sum(intensity == 3) / 60
  )
...
patient_summary %<>% 
  mutate(
    ...
    sedentary_prop__DWT = sedentary_hours__DWT / daytime_wear_hours, 
    light_prop__DWT = light_hours__DWT / daytime_wear_hours, 
    moderate_prop__DWT = moderate_hours__DWT / daytime_wear_hours, 
    active_prop__DWT = active_hours__DWT / daytime_wear_hours, 
    zero_steps_prop__AWT = zero_steps_hours__AWT / adherent_wear_hours, 
    sedentary_prop__AWT = sedentary_hours__AWT / adherent_wear_hours, 
    light_prop__AWT = light_hours__AWT / adherent_wear_hours, 
    moderate_prop__AWT = moderate_hours__AWT / adherent_wear_hours, 
    active_prop__AWT = active_hours__AWT / adherent_wear_hours, 
    zero_steps_prop__NAW = zero_steps_hours__NAW / nonadherent_wear_hours, 
    sedentary_prop__NAW = sedentary_hours__NAW / nonadherent_wear_hours, 
    light_prop__NAW = light_hours__NAW / nonadherent_wear_hours, 
    moderate_prop__NAW = moderate_hours__NAW / nonadherent_wear_hours, 
    active_prop__NAW = active_hours__NAW / nonadherent_wear_hours, 
    low_pa_prop__DWT = (sedentary_hours__DWT + light_hours__DWT) / daytime_wear_hours, 
    hi_pa_prop__DWT = (moderate_hours__DWT + active_hours__DWT) / daytime_wear_hours, 
    low_pa_prop__AWT = (sedentary_hours__AWT + light_hours__AWT) / adherent_wear_hours, 
    hi_pa_prop__AWT = (moderate_hours__AWT + active_hours__AWT) / adherent_wear_hours, 
    low_pa_prop__NAW = (sedentary_hours__NAW + light_hours__NAW) / nonadherent_wear_hours, 
    hi_pa_prop__NAW = (moderate_hours__NAW + active_hours__NAW) / nonadherent_wear_hours,
...
```{r}
post_pred = data.frame(hi_pa_prop__DWT = seq(0,0.3,0.01))
...
daily_data <- minute_data %>% 
  ...
  summarise(
    ...
    sedentary_prop = sum(intensity == 0, na.rm = T) / adherent_mins, 
    light_prop = sum(intensity == 1, na.rm = T) / adherent_mins, 
    moderate_prop = sum(intensity == 2, na.rm = T) / adherent_mins, 
    active_prop = sum(intensity == 3, na.rm = T) / adherent_mins, 
    low_pa_prop = sum(intensity <= 1, na.rm = T) / adherent_mins, 
    hi_pa_prop = sum(intensity >= 2, na.rm = T) / adherent_mins
    ) %>% ungroup() %>% 
  mutate(index = as.numeric(date - min_date + 1)) %>% 
  select(
    id, index, date, adherent_hours, 
    mean_HR, sd_HR, zero_step_prop, 
    sedentary_prop, light_prop, moderate_prop, active_prop, 
...
daily_summary <- daily_data %>% 
  ...
  summarise(
    ...
    mu_sedentary_prop = mean(sedentary_prop, na.rm = T), 
    sd_sedentary_prop = sd(sedentary_prop, na.rm = T), 
    mu_light_prop = mean(light_prop, na.rm = T), 
    sd_light_prop = sd(light_prop, na.rm = T), 
    mu_moderate_prop = mean(moderate_prop, na.rm = T), 
    sd_moderate_prop = sd(moderate_prop, na.rm = T), 
    mu_active_prop = mean(active_prop, na.rm = T), 
    sd_active_prop = sd(active_prop, na.rm = T), 
  ...
  mutate(
    ...
    sd_error_sedentary_prop = 1.96 * sd_sedentary_prop / sqrt(adherent_days), 
    sd_error_light_prop = 1.96 * sd_light_prop / sqrt(adherent_days), 
    sd_error_moderate_prop = 1.96 * sd_moderate_prop / sqrt(adherent_days), 
    sd_error_active_prop = 1.96 * sd_active_prop / sqrt(adherent_days), 
...
daily_data__AWT <- minute_data %>% 
  ...
  summarise(
    ...
    sedentary = sum(intensity == 0, na.rm = T), 
    light = sum(intensity == 1, na.rm = T), 
    moderate = sum(intensity == 2, na.rm = T), 
...
patient_summary %<>% 
  select(
    "id", "min_date", "max_date", "daytime_hours", 
    "daytime_wear_hours", "adherent_wear_hours", "nonadherent_wear_hours", 
    "daytime_wear_pct", "nonadherent_ratio", 
    "min_HR__AWT", "min_HR__NAW", 
    "max_HR__AWT", "max_HR__NAW", 
    "mean_HR__AWT", "mean_HR__NAW", 
    "max_pos_steps__AWT", "max_pos_steps__NAW", 
    "mean_pos_steps__AWT", "mean_pos_steps__NAW", 
    "zero_steps_prop__AWT", "zero_steps_prop__NAW", 
    "sedentary_prop__AWT", "sedentary_prop__NAW", 
    "light_prop__AWT", "light_prop__NAW", 
    "moderate_prop__AWT", "moderate_prop__NAW", 
    "active_prop__AWT", "active_prop__NAW", 
    "low_pa_prop__AWT", "low_pa_prop__NAW", 
    "hi_pa_prop__AWT", "hi_pa_prop__NAW",
    "low_pa_prop_ratio"
  )

patient_summary %>% 
  mutate(
    across(contains("wear_hours"), round, digits = 2), 
    daytime_wear_pct = round(daytime_wear_pct, 2), 
    across(contains("AWT"), round, digits = 2), 
    across(contains("NAW"), round, digits = 2), 
    across(contains("ratio"), round, digits = 2)
  ) %>% 
...
```{r}
 %>% 
  mutate(
    across(contains("mu"), function(x) round(x, digits = 2)), 
    across(contains("sd"), function(x) round(x, digits = 2))
  )
write_csv(, "daily_summary.csv")
```
...
minute_data %>% 
  ...
  filter(is_adherent_wear) %>% 
  ggplot() +
  geom_point(aes(x = steps, y = HR, col = intensity)) + 
  facet_grid(~ id)
```
